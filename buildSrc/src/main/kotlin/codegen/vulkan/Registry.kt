		...
		.map { extNode ->
			Extension(
				...
				extNode.getChildren("require").map { requireNode ->
					Require(
						requireNode.getChildren("enum")
							.filter { it.hasAttribute("extends") && !it.hasAttribute("alias") }.map {
								Enum(
									it.getAttribute("name"),
									it.getAttribute("extends"),
									it.getAttribute("value").toIntOrNull() ?: it.getAttribute("offset").toIntOrNull()
									?: it.getAttribute("bitpos").toInt()
								)
							}.toList(),
						requireNode.getChildren("enum")
							.filter { !it.hasAttribute("extends") }
