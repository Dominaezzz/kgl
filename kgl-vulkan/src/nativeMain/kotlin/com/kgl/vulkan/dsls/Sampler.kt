...
import cvulkan.VkSamplerCreateInfo
...
actual class SamplerCreateInfoBuilder(internal val target: VkSamplerCreateInfo) {
	...
	actual var anisotropyEnable: Boolean
		get() = target.anisotropyEnable.toBoolean()
		set(value) {
	...
	actual var compareEnable: Boolean
		get() = target.compareEnable.toBoolean()
		set(value) {
	...
	actual var unnormalizedCoordinates: Boolean
		get() = target.unnormalizedCoordinates.toBoolean()
		set(value) {
			target.unnormalizedCoordinates = value.toVkType()
		}

	internal fun init() {
		target.sType = cvulkan.VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO
		target.pNext = null
		target.flags = 0U
	}
}
