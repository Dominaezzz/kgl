...
import cvulkan.VkDisplayModeCreateInfoKHR
import cvulkan.VkDisplayModeParametersKHR
import cvulkan.VkDisplayPlaneInfo2KHR

actual class DisplayModeParametersKHRBuilder(internal val target: VkDisplayModeParametersKHR) {
	actual var refreshRate: UInt
		get() = target.refreshRate
		set(value) {
...
actual class DisplayModeCreateInfoKHRBuilder(internal val target: VkDisplayModeCreateInfoKHR) {
	...
	internal fun init() {
		target.sType = cvulkan.VK_STRUCTURE_TYPE_DISPLAY_MODE_CREATE_INFO_KHR
		target.pNext = null
		target.flags = 0U
	}
}
...
actual class DisplayPlaneInfo2KHRBuilder(internal val target: VkDisplayPlaneInfo2KHR) {
	...
	internal fun init(mode: DisplayModeKHR) {
		target.sType = cvulkan.VK_STRUCTURE_TYPE_DISPLAY_PLANE_INFO_2_KHR
		target.pNext = null
