...
actual class DescriptorSetLayoutCreateInfoBuilder(internal val target: VkDescriptorSetLayoutCreateInfo) {
	...
	actual fun bindings(block: DescriptorSetLayoutBindingsBuilder.() -> Unit) {
		...
		target.pBindings = targets.mapToStackArray()
		target.bindingCount = targets.size.toUInt()
	}

	internal fun init() {
		target.sType = cvulkan.VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO
		target.pNext = null
	}
}
...
actual class DebugUtilsObjectNameInfoEXTBuilder(internal val target: VkDebugUtilsObjectNameInfoEXT) {
	...
	internal fun init() {
		target.sType = cvulkan.VK_STRUCTURE_TYPE_DEBUG_UTILS_OBJECT_NAME_INFO_EXT
		target.pNext = null
	}
}
...
actual class DebugUtilsLabelEXTBuilder(internal val target: VkDebugUtilsLabelEXT) {
	actual var labelName: String?
		get() = target.pLabelName?.toKString()
		set(value) {
	...
	internal fun init() {
		target.sType = cvulkan.VK_STRUCTURE_TYPE_DEBUG_UTILS_LABEL_EXT
		target.pNext = null
	}
}
...
actual class AccelerationStructureInfoNVBuilder(internal val target: VkAccelerationStructureInfoNV) {
	...
	actual fun geometries(block: GeometryNVsBuilder.() -> Unit) {
		...
		target.pGeometries = targets.mapToStackArray()
		target.geometryCount = targets.size.toUInt()
	}

	internal fun init() {
		target.sType = cvulkan.VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_INFO_NV
		target.pNext = null
	}
}
