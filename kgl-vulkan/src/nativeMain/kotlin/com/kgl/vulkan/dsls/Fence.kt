...
import cvulkan.VkFenceCreateInfo
import cvulkan.VkFenceGetFdInfoKHR
import cvulkan.VkImportFenceFdInfoKHR
...
actual class FenceCreateInfoBuilder(internal val target: VkFenceCreateInfo) {
	...
	internal fun init() {
		target.sType = cvulkan.VK_STRUCTURE_TYPE_FENCE_CREATE_INFO
		target.pNext = null
	}
}
...
actual class FenceGetFdInfoKHRBuilder(internal val target: VkFenceGetFdInfoKHR) {
	...
	internal fun init(fence: Fence) {
		target.sType = cvulkan.VK_STRUCTURE_TYPE_FENCE_GET_FD_INFO_KHR
		target.pNext = null
...
actual class ImportFenceFdInfoKHRBuilder(internal val target: VkImportFenceFdInfoKHR) {
	...
	internal fun init(fence: Fence) {
		target.sType = cvulkan.VK_STRUCTURE_TYPE_IMPORT_FENCE_FD_INFO_KHR
		target.pNext = null
		target.fence = fence.toVkType()
	}
}
