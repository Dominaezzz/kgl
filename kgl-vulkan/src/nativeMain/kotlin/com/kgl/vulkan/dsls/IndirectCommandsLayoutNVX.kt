...
import cvulkan.VkIndirectCommandsLayoutCreateInfoNVX
import cvulkan.VkIndirectCommandsLayoutTokenNVX
...
actual class IndirectCommandsLayoutCreateInfoNVXBuilder(internal val target: VkIndirectCommandsLayoutCreateInfoNVX) {
	...
	actual fun tokens(block: IndirectCommandsLayoutTokenNVXsBuilder.() -> Unit) {
		...
		target.pTokens = targets.mapToStackArray()
		target.tokenCount = targets.size.toUInt()
	}

	internal fun init() {
		target.sType = cvulkan.VK_STRUCTURE_TYPE_INDIRECT_COMMANDS_LAYOUT_CREATE_INFO_NVX
		target.pNext = null
	}
}
