...
actual enum class ShaderStage(override val value: VkShaderStageFlagBits) : VkFlag<ShaderStage> {
	...
	companion object {
		private val enumLookUpMap: Map<UInt, ShaderStage> = enumValues<ShaderStage>().associateBy({
			it.value
		})

		fun fromMultiple(value: VkShaderStageFlagBits): VkFlag<ShaderStage> = VkFlag(value)

		fun from(value: VkShaderStageFlagBits): ShaderStage = enumLookUpMap[value]!!
	}
}
