...
actual class GeometryTrianglesNVBuilder(internal val target: VkGeometryTrianglesNV) {
	...
	actual var transformOffset: ULong
		get() = target.transformOffset().toULong()
		set(value) {
	...
	internal fun init(
		...
		target.sType(NVRayTracing.VK_STRUCTURE_TYPE_GEOMETRY_TRIANGLES_NV)
		target.pNext(0)
...
actual class GeometryAABBNVBuilder(internal val target: VkGeometryAABBNV) {
	...
	internal fun init(aabbData: Buffer?) {
		target.sType(NVRayTracing.VK_STRUCTURE_TYPE_GEOMETRY_AABB_NV)
		target.pNext(0)
...
actual class GeometryNVBuilder(internal val target: VkGeometryNV) {
	...
	internal fun init() {
		target.sType(NVRayTracing.VK_STRUCTURE_TYPE_GEOMETRY_NV)
		target.pNext(0)
	}
}
...
actual class AccelerationStructureCreateInfoNVBuilder(internal val target: VkAccelerationStructureCreateInfoNV) {
	actual var compactedSize: ULong
		get() = target.compactedSize().toULong()
		set(value) {
	...
	internal fun init() {
		target.sType(NVRayTracing.VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_CREATE_INFO_NV)
		target.pNext(0)
	}
}
